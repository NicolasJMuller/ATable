...
htmldebut('Juliette et Nicolas','Résultats');
navigation('recherches');

$search = isset($_GET['searchIt']) ? mysqli_real_escape_string($connect, $_GET['searchIt']) : '';

// Demandes
$requeteDem = "SELECT R.*, C.*, E.*
FROM AT_REQUESTS R
...
$resultDem = mysqli_query($connect ,$requeteDem);

// Offres
$requeteOff = "SELECT O.*, P.*
FROM AT_OFFERS O
JOIN AT_PROVIDERS P ON O.PR_Code = P.PR_Code
WHERE OF_Label LIKE '%$search%' OR OF_Description LIKE '%$search%'";
$resultOff = mysqli_query($connect ,$requeteOff);

// Affichage
?>

<section class="container py-5">
    <h2 class="text-center mb-4">Résultats de votre recherche : <em><?= htmlspecialchars($search) ?></em></h2>

    <div class="text-center mt-5 mb-3">
        <h2 class="fw-bold">Résultats des Demandes</h2>
        <hr class="w-25 mx-auto" style="border-top: 2px solid #F2C3AF;">
    </div>

    <?php
    if (mysqli_num_rows($resultDem) == 0) {
        echo '<div class="w-100 d-flex justify-content-center"><p class="text-muted text-center">Aucune demande ne correspond à votre recherche.</p></div>';
    } else {
        echo '<div class="row justify-content-center">';
        while ($row = mysqli_fetch_assoc($resultDem)) {
    ...
    <div class="text-center mt-5 mb-3">
        <h2 class="fw-bold">Résultats des Offres</h2>
        <hr class="w-25 mx-auto" style="border-top: 2px solid #A3C7D6;">
    </div>

    <?php
    if (mysqli_num_rows($resultOff) == 0) {
        echo '<div class="w-100 d-flex justify-content-center"><p class="text-muted text-center">Aucune offre ne correspond à votre recherche.</p></div>';
    } else {
        echo '<div class="row justify-content-center">';
        while ($row = mysqli_fetch_assoc($resultOff)) {
            echo '<article class="col-sm-6 col-md-4 col-lg-3 m-5">
                <div class="card">
